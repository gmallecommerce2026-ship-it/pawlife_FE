...
const LanguageOption = ({ label, subLabel, isSelected, onPress, isLast = false }: LanguageOptionProps) => (
  ...
  >
    <View className="flex-col">
      <Text className={`text-base font-medium ${isSelected ? 'text-[#ffa053]' : 'text-gray-900'}`}>
        {label}
      </Text>
      {subLabel && (
          <Text className="text-sm text-gray-500 mt-1">{subLabel}</Text>
      )}
    </View>
    {isSelected && (
      <Feather name="check" size={24} color="#ffa053" />
    )}
  </TouchableOpacity>
);
  ...
  return (
    <View className="flex-1 bg-[#F9FAFB]">
      <SafeAreaView className="flex-1" edges={['top', 'bottom']}>
        ...
        {/* --- HEADER --- */}
        <View className="flex-row items-center px-4 py-2 mb-2 relative bg-white pb-4 shadow-sm z-10 border-b border-gray-100">
            ...
            <View className="absolute left-0 right-0 items-center justify-center pointer-events-none">
                ...
                <Text className="text-xl font-bold text-gray-900">{t('Language')}</Text>
            </View>
        </View>
