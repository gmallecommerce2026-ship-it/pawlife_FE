...
export default function HomeScreen() {
  ...
  const { location, errorMsg } = useLocation(); // Hook lấy toạ độ
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      // Chỉ gọi API data nếu đã có location, dùng isSilentRefresh = true để không bị chớp màn hình (loading) liên tục
      if (location !== null || errorMsg !== null) {
          loadHomeData(location?.lat, location?.lng, true);
      }
    }, [location, user?.id]) 
  );
  // Tách hàm loadHomeData ra riêng để tái sử dụng
  const loadHomeData = async (currentLat?: number, currentLng?: number, isSilentRefresh = false) => {
    try {
      ...
      // FIX LỖI 1: Nếu muốn nhân bản để UI đẹp, BẮT BUỘC phải tạo ID giả (unique) để tránh lỗi FlatList
  ...
  useEffect(() => {
    if (location === null && errorMsg === null) return;

    const initLoad = async () => {
      setIsLoading(true);
      // Truyền toạ độ mới nhất vào hàm
      await loadHomeData(location?.lat, location?.lng);
      setIsLoading(false);
    };
    initLoad();
  }, [location, errorMsg, user?.id]);

  const onRefresh = async () => {
    setRefreshing(true);
    // Truyền toạ độ mới nhất vào hàm khi vuốt làm mới
    await loadHomeData(location?.lat, location?.lng); 
    setRefreshing(false);
  };
    useEffect(() => {
    // Hiệu ứng nghiêng nhẹ qua lại (Wiggle)
    rotation.value = withRepeat(
      withDelay(
        4000, // Nghỉ 4 giây trước khi lặp lại
        withSequence(
    ...
    // Hiệu ứng nảy nhẹ lên xuống (Bounce) đồng thời
  ...
  return (
    <View className="flex-1 bg-[#FAFAFA]">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
                    ...
                    <View className="flex-row justify-between items-start">
                        ...
                        <View className="flex-row gap-5 items-center mt-2">
                             ...
                             <TouchableOpacity activeOpacity={0.7} className="relative" onPress={() => router.push('/notifications')} >
                                ...
                                {/* Chỉ hiển thị dấu chấm đỏ nếu tồn tại thông báo chưa đọc */}
                                {hasUnread && (
        ...
        {/* --- 2. MAIN CONTENT BODY --- */}
        <View className="flex-1 bg-[#FAFAFA] rounded-t-[40px] -mt-14 px-6 pt-2 pb-6 overflow-hidden">
            <AnimatedTouchableOpacity 
                activeOpacity={0.8} 
                onPress={() => router.push('/scan')}
                style={animatedStyle}
            >
                <View className="bg-[#FFF8F0] p-6 rounded-[32px] shadow-sm shadow-orange-100 flex-row items-center border border-orange-100 mt-6">
                    <View className="w-16 h-16 bg-[#ffc99e] rounded-2xl items-center justify-center mr-5 shadow-lg shadow-orange-200">
        ...
        <View className="mt-2">
            ...
            {pets.length === 0 ? (
                ...
                <FlatList
                    ...
                    data={pets}
                    // FIX TẠI ĐÂY: Sử dụng fakeId nếu là hàng clone, nếu không thì dùng id gốc
                    ...
                    renderItem={({ item: pet }) => {
                        ...
                        return (
                            <TouchableOpacity 
                                className="w-40 h-52 rounded-[24px] overflow-hidden relative bg-gray-200" 
                                activeOpacity={0.9}
                                onPress={() => router.push({
                                    pathname: '/pet-detail-modal',
                                    params: { 
                                        id: pet.id, name: pet.name, gender: pet.gender || 'male', 
                                        distance: pet.distance || 'Gần đây', image: petImageUrl,
                                        age: pet.age || 'Unknown', breed: pet.breed || 'Unknown Breed'
                                    }
                                })}
                            >
                                <Image source={{ uri: petImageUrl }} className="w-full h-full" resizeMode="cover" />
                                <View className="absolute bottom-0 w-full p-3 bg-black/30 pt-6">
                                    <View className="flex-row items-center justify-between">
                                        <Text className="text-white font-bold text-lg">{pet.name}</Text>
                                        <View className="bg-white/20 px-1.5 py-0.5 rounded-md backdrop-blur-md">
                                            <Ionicons name={pet.gender === 'FEMALE' ? 'female' : 'male'} size={12} color="white" />
                                        </View>
                                    </View>
                                    <View className="flex-row items-center mt-1">
                                        <Ionicons name="location-sharp" size={10} color="#FB923C" />
                                        <Text className="text-gray-200 text-[10px] ml-1">{pet.distance || 'Gần đây'}</Text>
                                    </View>
                                </View>
                            </TouchableOpacity>
                        )
        ...
        <View className="mt-8 mb-6"> 
             ...
             <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24, paddingVertical: 5, gap: 16 }}>
                 {events.map((event) => {
                     // Parse Ngày tháng để hiển thị UI
