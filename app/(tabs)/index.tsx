...
export default function HomeScreen() {
  ...
  const textContainerAnimatedStyle = useAnimatedStyle(() => {
    const translateY = interpolate(scrollY.value, [0, SCROLL_DISTANCE], [0, 48], Extrapolation.CLAMP);
    ...
    <View className="flex-1 bg-[#ffffff]">
      ...
      <Animated.View style={[headerAnimatedStyle, { position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, backgroundColor: '#FFDDA2', overflow: 'hidden' }]} pointerEvents="box-none">
        ...
        <Animated.View 
            ...
            style={[
                cornerOverlayAnimatedStyle,
                {
                    ...
                    backgroundColor: 'rgba(232, 155, 90, 0.35)', 
                    zIndex: 2,
                }
            ]} 
        />
        ...
        <View className="px-6 w-full h-full" pointerEvents="box-none" style={{ paddingTop: insets.top + 10, zIndex: 10 }}>
            <View className="flex-row justify-between items-start z-20" pointerEvents="box-none">
                <TouchableOpacity 
                    activeOpacity={0.8}
                    onPress={() => router.push('/edit-profile')}
                >
                    <Animated.View style={[avatarAnimatedStyle, { backgroundColor: '#ffedd5', overflow: 'hidden' }]}>
        ...
        <View 
            className="absolute bottom-0 w-full bg-white rounded-t-[40px]" 
            style={{ height: CURVE_HEIGHT, zIndex: 15 }} 
            pointerEvents="none"
        />
      </Animated.View>
